...
// Har bir so'rovga qo'shiladigan umumiy header'lar (tenant + content-type)
function authHeaders(extra = {}) {
  return {
    "X-Company-Id": String(window.APP_CONFIG.COMPANY_ID),
    ...extra,
  };
}
...
async function loadFinance() {
  const summaryRes = await fetch(`${API}/finance/summary`, { headers: authHeaders() });
  ...
  const txRes = await fetch(`${API}/finance/transactions`, { headers: authHeaders() });
